...
(defcustom plain-org-academic-directory "~/org/wiki/"
...
(defvar plain-org-wiki-extra-dirs '("~/Academics/Notebook/astronomy-org/org/")
  "List of extra directories in addition to `plain-org-wiki-directory'.")

(defvar plain-org-wiki-extra-files nil
  "List of extra files.")

(defvar plain-org-academic-extra-dirs nil
  "List of extra directories in addition to `plain-org-wiki-directory'.")

(defvar plain-org-academic-extra-files nil
  "List of extra files.")
...
(defun plain-org-academic-files ()
  "Return cons cells for files in `plain-org-academic-directory'.
Each cons cell is a name and file path."
  (append
   (cl-mapcan #'plain-org-wiki-files-in-dir
              (cons plain-org-academic-directory plain-org-academic-extra-dirs))
   (mapcar #'plain-org-wiki--item plain-org-academic-extra-files)))
...
(defun plain-org-academic-find-file (x)
  "Open X as a file with org extension in `plain-org-academic-directory'."
  (when (consp x)
    (setq x (cdr x)))
  (with-ivy-window
    (if (file-exists-p x)
      ...
      (if (string-match "org$" x)
          (find-file
           (expand-file-name x plain-org-academic-directory))
        (find-file
         (expand-file-name
          (format "%s.org" x) plain-org-academic-directory))))))
...
(defun plain-org-academic-helm ()
  "Select an org-file to jump to."
  (interactive)
  (helm :sources
        '(((name . "Projects")
           (candidates . plain-org-academic-files)
           (action . plain-org-wiki-find-file))
          ((name . "Create org-academic")
           (dummy)
           (action . plain-org-wiki-find-file)))))
...
(defun plain-org-academic ()
  "Select an org-file to jump to."
  (interactive)
  (ivy-read "pattern: " (plain-org-academic-files)
            :history 'plain-org-wiki-history
            :action 'plain-org-wiki-find-file
            :caller 'plain-org-wiki))
